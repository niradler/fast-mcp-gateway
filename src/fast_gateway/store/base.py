...
implementation is :class:`fast_gateway.store.sqlite.SqliteStore`; Postgres, Redis,
...
class Store(Protocol):
    ...
    async def close(self) -> None:
        """Release backend resources (close connections). Called once at gateway
        shutdown; must be safe to call even if ``initialize`` never ran."""
        ...
